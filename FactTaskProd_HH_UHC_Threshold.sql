...
END AS VisitBetweenSOEandEOE,
	...
	-- BC-There may need to be logic adjustments depending on if we go based on status, or both status and dates	
	CASE 
		WHEN CEA.EpiEndOfEpisode <= CAST(GETDATE() -1 AS DATE) THEN 'Closed' 	
		ELSE 'Open'
...
--	hcpcs.BcHCPCS as HCPCS,
	EBP.HIPPS,
	CASE WHEN COALESCE(i.InsuranceTypeId, CEFS.CefsPsid)=25024 THEN 4 ELSE EBP.LupaThreshold END AS LupaThreshold_UHC, 
	EBP.LupaThreshold,
	--h.isLupa,
...
--AND billingPeriodStartDate = '20251009' 
 -- PatientName LIKE '%GOODYEAR, AVERY%'
  --and episodeStartDate = 20250906
--and bpKey = '941294-1'


    --Comment out the below to validate
--AND
--    (
...
--SELECT 
--  csv.CsvId, csv.CsvVisitNumber, csv.CsvStatus,
--  vs.vsStatus, vs.vsDescription, sc.ScBillable, sc.ScCode
--FROM dbo.ClientSchedVisits csv
--LEFT JOIN dbo.VisitStatuses vs 
--  ON csv.CsvStatus = vs.vsStatus AND csv.SourceSystem = vs.SourceSystem
--LEFT JOIN dbo.ServiceCodes sc 
--  ON csv.CsvScId = sc.ScId AND csv.SourceSystem = sc.SourceSystem
--WHERE csv.CsvEpiId = 932365
-- AND csv.CsvVisitNumber IN (4,5);


--SELECT * FROM dbo.ClientEpisodeVisitsAll WHERE CevEpiId = 932365;
--SELECT * FROM dbo.ClientSchedVisits WHERE CsvEpiId = 932365;
...
--WHERE csv.CsvEpiId = 932365
--  --AND csv.CsvVisitNumber IN (4,5)
--  ORDER BY CsvVisitNumber asc

--  SELECT DISTINCT csv.SourceSystem, csv.CsvVisitNumber
--FROM dbo.ClientSchedVisits csv
--WHERE csv.CsvEpiId = 932365
--ORDER BY csv.CsvVisitNumber;

--SELECT * FROM dbo.ClientEpisodeVisitsAll CEV
--   WHERE [CevEpiId] = 932365
--   ORDER BY [CevVisitNumber] ASC
...
--    ON csv.CsvSchedDate BETWEEN rp.RpSoeEffectiveFrom AND rp.RpSoeEffectiveTo
--   AND rp.RpActive = 'Y'
--   AND rp.RpSltId = 1
--   AND rp.RpRptId = 5
--   AND csv.SourceSystem = rp.SourceSystem
--WHERE csv.CsvEpiId = 932365
--GROUP BY csv.CsvId, csv.CsvVisitNumber
--HAVING COUNT(*) > 1;
