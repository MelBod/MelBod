     ...
     SELECT CONVERT(CHAR(8), /* coalesce( csv.CsvAnticipatedDate, */ csv.CsvSchedDate, 112) AS skScheduleDateKey
          ...
          ,CASE 
               WHEN COALESCE(i.InsuranceTypeId, CEFS.CefsPsid) = 25024
                    THEN 4
               ELSE EBP.LupaThreshold
               END AS LupaThreshold_UHC
          ,EBP.LupaThreshold
          ,
          --h.isLupa,
...
SELECT DISTINCT episodeId AS 'Episode_ID'
     ,CONVERT(DATE, episodeStartDate, 112)  AS EpisodeStartDate
     ,CONVERT(DATE, episodeEndDate, 112) AS EpisodeEndDate
     ,epistatus AS 'Episode_Status'
   ,CONVERT(DATE, skScheduleDateKey, 112) AS ScheduleDate


     ,taskStatus AS 'Visit_Status'
     ,taskType AS 'Visit Type'
     ,PatientName
     ,taskJobCode AS 'Visit Job Code'
     ,taskCode AS 'Service Code'
     ,taskDesc AS 'Service Code Desc'
     ,CsvVisitNumber AS 'Visit_Number'
     ,CompletedVisits
     ,ScheduledVisits
     ,Visitbillable
...
WHERE EpisodeID = 912119
	 --PatientName = 'SAUNDERS, LYNN'
	      --AND TaskType = 'AIDE'
     --AND CsvVisitNumber = 3
